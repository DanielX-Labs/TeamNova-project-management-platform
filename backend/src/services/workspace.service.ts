...
export const deleteWorkspaceService = async (
  ...
  try {
    ...
    if (!workspace) {
      // DELETE is idempotent. Repair any stale references left by an already
      // deleted workspace and return the user's next available workspace.
      await MemberModel.deleteMany({ workspaceId, userId }).session(session);
      ...
      if (user.currentWorkspace?.equals(workspaceId)) {
        const remainingMembership = await MemberModel.findOne({ userId })
          .sort({ joinedAt: 1 })
          .session(session);
        user.currentWorkspace = remainingMembership
    ...
    if (user?.currentWorkspace?.equals(workspaceId)) {
      // Pick the replacement before removing this workspace's memberships.
