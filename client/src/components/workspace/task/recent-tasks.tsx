      ...
      <ul role="list" className="divide-y divide-gray-200">
        {tasks.map((task) => {
          ...
          return (
            <li
              key={task._id}
              className="flex items-center justify-between p-4 transition-colors hover:bg-[#F8FAFC]"
            >
              {/* Task Info */}
              <div className="flex flex-col space-y-1 flex-grow">
                <span className="text-sm font-medium capitalize text-[#64748B]">
                  {task.taskCode}
                </span>
                <p className="text-md truncate font-semibold text-[#0F172A]">
                  {task.title}
                </p>
